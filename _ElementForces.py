...
def get_G1(self):
    # Compute G_1^INT.
    self.G_1 = np.einsum('ij..., ...i', self.Bu, np.einsum('...I, ...', self.FPK.reshape((8,9)), self.weights*self.j), dtype=np.float64)
    return

@register_method
def get_G2(self, Parameters):
    # Compute G_2^INT.
    self.G_2 = np.einsum('ij..., ...i', self.Nu, np.einsum('...i, ...', self.rho, self.weights*self.j*Parameters.grav), dtype=np.float64)
    return